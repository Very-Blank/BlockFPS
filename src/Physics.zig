    ...
    while (iterator.next()) |body1| {
        ...
        while (inner_iterator.next()) |body2| {
            if (collision(body1, body2)) {
                ...
                body1_pos.* = body1_pos.add(amount.scale(ratio));
                body2_pos.* = body2_pos.add(amount.scale(1.0 - ratio).negate());
...
pub inline fn collision(
    ...
    inline for (.{ "x", "y", "z" }) |axis| {
        if (!collide(
            @field(body2_pos, axis) - @field(body2_col, axis) / 2.0,
            @field(body1_pos, axis) - @field(body1_col, axis) / 2.0,
            @field(body1_pos, axis) + @field(body1_col, axis) / 2.0,
        ) and !collide(
            @field(body2_pos, axis) + @field(body2_col, axis) / 2.0,
            @field(body1_pos, axis) - @field(body1_col, axis) / 2.0,
